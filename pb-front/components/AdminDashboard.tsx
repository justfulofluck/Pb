...
import { Product, EventBlog, HeroSlide, BlogPost } from '../types';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [uiView, setUiView] = useState<'hero'>('hero');
  ...
  // Distributor Apps State (Mock Data)
  const [distributorApps, setDistributorApps] = useState([
    { id: 'd-1', company: 'Green Earth Foods', contact: 'Rajesh Kumar', region: 'Mumbai Suburbs', status: 'Pending', email: 'rajesh@greenearth.com', phone: '+91 98765 43210', message: 'We have a network of 45 organic stores in Mumbai and Thane. Looking to expand into healthy snacks.', applied: '2 hours ago' },
    { id: 'd-2', company: 'FitLife Distribution', contact: 'Anita Desai', region: 'Bangalore East', status: 'Approved', email: 'anita@fitlife.in', phone: '+91 98765 43211', message: 'Specializing in gyms and fitness centers. We want to stock Pinobite in our 20+ partner gyms.', applied: '1 day ago' },
    { id: 'd-3', company: 'Urban Basket', contact: 'Vikram Singh', region: 'Delhi NCR', status: 'Rejected', email: 'vikram@urbanbasket.com', phone: '+91 98765 43212', message: 'General grocery distribution for supermarkets.', applied: '3 days ago' },
  ]);
  const [viewingDistributor, setViewingDistributor] = useState<{
    id: string;
    company: string;
    contact: string;
    region: string;
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex font-display">
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scroll" onClick={() => { setShowNotifications(false); setShowSettings(false); }}>
          ...
          {activeTab === 'ui-settings' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                <div>
                  <h3 className="text-xl font-black uppercase text-slate-900">Home Page Banners</h3>
                  <p className="text-sm text-slate-500 font-medium">Manage your hero sliders and promotional banners.</p>
                </div>
                <button
                  onClick={openAddSlideModal}
                  className="px-6 py-3 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest hover:bg-primary transition-all shadow-lg hover:shadow-xl flex items-center gap-2"
                >
                  <span className="material-symbols-outlined">add_photo_alternate</span>
                  Add New Slide
                </button>
              ...
              <div className="space-y-4">
                {slides.map((slide, index) => (
                  <div key={slide.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-center group hover:border-primary/50 transition-colors">

                    {/* Handle & Index */}
                    <div className="hidden md:flex flex-col items-center gap-2 text-slate-300">
                      <button onClick={() => moveSlide(index, 'up')} disabled={index === 0} className="hover:text-primary disabled:opacity-30"><span className="material-symbols-outlined">expand_less</span></button>
                      <span className="font-mono font-bold text-xs">{index + 1}</span>
                      <button onClick={() => moveSlide(index, 'down')} disabled={index === slides.length - 1} className="hover:text-primary disabled:opacity-30"><span className="material-symbols-outlined">expand_more</span></button>
                    </div>

                    {/* Thumbnail */}
                    <div className="w-full md:w-64 h-32 rounded-xl overflow-hidden relative flex-shrink-0 bg-slate-100">
                      <img src={slide.image} alt={slide.category} className="w-full h-full object-cover" />
                      <div className={`absolute top-2 left-2 px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${slide.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-500'}`}>
                    ...
                    <div className="flex-1 min-w-0 text-center md:text-left">
                      <div className="flex items-center justify-center md:justify-start gap-3 mb-1">
                        <span className={`w-3 h-3 rounded-full ${slide.bgColor.replace('bg-', 'bg-')}`}></span>
                        <p className="text-xs font-black uppercase tracking-widest text-slate-500">{slide.category}</p>
                      </div>
                      <h4 className="text-lg font-black uppercase text-slate-900 truncate">{slide.headline}</h4>
                      <p className="text-sm text-slate-500 line-clamp-1 mt-1">{slide.description}</p>
                      <div className="mt-3 flex items-center justify-center md:justify-start gap-4 text-xs font-bold text-slate-400">
                        <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">palette</span> {slide.bgColor.replace('bg-', '').replace('-50', '')} Theme</span>
                        <span className="flex items-center gap-1"><span className="material-symbols-outlined text-sm">touch_app</span> CTA: {slide.cta}</span>
                      </div>
                    </div>

                    {/* Actions */}
                    <div className="flex items-center gap-2 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 w-full md:w-auto justify-center">
                      <button
                        onClick={() => toggleSlideStatus(slide.id)}
                        className={`p-2 rounded-lg transition-colors ${slide.isActive ? 'text-green-600 hover:bg-green-50' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}
          ...
          {activeTab === 'distributors' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { label: 'Total Applications', value: distributorApps.length, color: 'bg-slate-100 text-slate-600', icon: 'folder_shared' },
                  { label: 'Pending Review', value: distributorApps.filter(d => d.status === 'Pending').length, color: 'bg-orange-100 text-orange-600', icon: 'pending' },
                  { label: 'Active Partners', value: distributorApps.filter(d => d.status === 'Approved').length, color: 'bg-green-100 text-green-600', icon: 'handshake' },
                ].map((stat, i) => (
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                  ...
                  <div className="flex gap-2">
                    <select className="px-4 py-2 rounded-lg border border-slate-200 text-sm font-bold text-slate-600 bg-white">
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="p-4 text-xs font-black uppercase text-slate-500 tracking-widest">Company</th>
                      <th className="p-4 text-xs font-black uppercase text-slate-500 tracking-widest">Contact</th>
                      <th className="p-4 text-xs font-black uppercase text-slate-500 tracking-widest">Region</th>
                      <th className="p-4 text-xs font-black uppercase text-slate-500 tracking-widest">Status</th>
                      <th className="p-4 text-xs font-black uppercase text-slate-500 tracking-widest text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {distributorApps.map(app => (
                      <tr key={app.id} className="hover:bg-slate-50 transition-colors">
                        <td className="p-4">
                          <p className="font-bold text-slate-900 text-sm">{app.company}</p>
                          <p className="text-xs text-slate-400 font-medium">Applied {app.applied}</p>
                        </td>
                        <td className="p-4">
                          <p className="text-sm font-bold text-slate-700">{app.contact}</p>
                          <p className="text-xs text-slate-500">{app.email}</p>
                        </td>
                        <td className="p-4 text-sm font-bold text-slate-600">{app.region}</td>
                        <td className="p-4">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${app.status === 'Approved' ? 'bg-green-100 text-green-700' :
                        ...
                        <td className="p-4 text-right flex items-center justify-end gap-2">
                          <button
                            onClick={() => setViewingDistributor(app)}
                            className="p-2 rounded-lg text-slate-400 hover:text-primary hover:bg-primary/5 transition-colors"
                          ...
                          {app.status === 'Pending' && (
                            <>
                              <button
                                onClick={() => {
                                  setDistributorApps(prev => prev.map(p => p.id === app.id ? { ...p, status: 'Approved' } : p));
                                }}
                              ...
                              <button
                                onClick={() => {
                                  setDistributorApps(prev => prev.map(p => p.id === app.id ? { ...p, status: 'Rejected' } : p));
                                }}
                                className="p-2 rounded-lg text-slate-400 hover:text-red-500 hover:bg-red-50 transition-colors"
                                title="Reject"
                              >
                          ...
                          <button
                            onClick={() => {
                              if (confirm('Are you sure you want to delete this application?')) {
                                setDistributorApps(prev => prev.filter(p => p.id !== app.id));
                              }
                            }}
          ...
          {activeTab !== 'overview' && activeTab !== 'products' && activeTab !== 'events' && activeTab !== 'orders' && activeTab !== 'ui-settings' && activeTab !== 'visitor-forms' && activeTab !== 'blogs' && activeTab !== 'distributors' && (
      ...
      {viewingDistributor && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setViewingDistributor(null)} />
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl relative z-10 p-8 animate-in zoom-in duration-300">
            <div className="flex justify-between items-start mb-6 border-b border-slate-100 pb-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-2xl font-black uppercase text-slate-900">{viewingDistributor.company}</h3>
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${viewingDistributor.status === 'Approved' ? 'bg-green-100 text-green-700' :
                      viewingDistributor.status === 'Pending' ? 'bg-orange-100 text-orange-700' :
                        'bg-red-100 text-red-700'
                    }`}>
                    {viewingDistributor.status}
                  </span>
                </div>
                <p className="text-sm font-bold text-slate-500">Applied {viewingDistributor.applied}</p>
              </div>
              <button onClick={() => setViewingDistributor(null)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center hover:bg-slate-200"><span className="material-symbols-outlined">close</span></button>
            </div>

            <div className="grid md:grid-cols-2 gap-8 mb-8">
              <div>
                <h4 className="font-bold text-xs uppercase text-slate-400 tracking-widest mb-2">Contact Details</h4>
                <p className="font-bold text-slate-900">{viewingDistributor.contact}</p>
                <p className="text-sm text-slate-600">{viewingDistributor.email}</p>
                <p className="text-sm text-slate-600">{viewingDistributor.phone}</p>
              </div>
              <div>
                <h4 className="font-bold text-xs uppercase text-slate-400 tracking-widest mb-2">Target Region</h4>
                <p className="font-bold text-slate-900">{viewingDistributor.region}</p>
              </div>
            </div>

            <div className="bg-slate-50 p-6 rounded-2xl mb-8">
              <h4 className="font-bold text-xs uppercase text-slate-400 tracking-widest mb-3">Network & Stats</h4>
              <p className="text-sm text-slate-700 leading-relaxed font-medium">{viewingDistributor.message}</p>
            </div>

            <div className="flex justify-end gap-4">
              {viewingDistributor.status === 'Pending' ? (
                <>
                  <button
                    onClick={() => {
                      setDistributorApps(prev => prev.map(p => p.id === viewingDistributor.id ? { ...p, status: 'Rejected' } : p));
                      setViewingDistributor(null);
                    }}
                    className="px-6 py-3 rounded-xl font-bold text-red-500 hover:bg-red-50 border border-transparent hover:border-red-200 transition-all uppercase tracking-widest text-xs"
                  >
                    Reject Application
                  </button>
                  <button
                    onClick={() => {
                      setDistributorApps(prev => prev.map(p => p.id === viewingDistributor.id ? { ...p, status: 'Approved' } : p));
                      setViewingDistributor(null);
                    }}
                    className="px-8 py-3 rounded-xl bg-primary text-white font-black uppercase tracking-widest text-xs hover:shadow-lg transition-all"
                  >
                    Approve Partner
                  </button>
                </>
                ...
                <button
                  onClick={() => setViewingDistributor(null)}
